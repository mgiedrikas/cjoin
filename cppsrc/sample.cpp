...
vector <Join> parseSqlGetJoins(string sql) {
    ...
    if (sql.find(delimiter) == string::npos) {
        delimiter = "join ? ";
        if (sql.find(delimiter) == string::npos) {
            throw "no 'JOIN ? ' or 'join ? ' not found";
        }
    }
...
//        cout << "first table: " << join.GetFirstTable() << " second table: " << join.GetSecondTable() << endl;
        delim = " AND ";
        if (v[1].find(delim) == string::npos) {
            delim = " and ";
            if (v[1].find(delim) == string::npos) {
                throw "no 'AND' or 'and' not found";
            }
        }
...
Napi::Object
JoinObjects(Napi::Env &env, Object obj1, Object obj2, Join join) {
